...
const ModalCategory = ({ active, setActive, canSpendSum, setCanSpendSum }) => {
  ...
  return (
    <div className={active ? "modalCategory active" : "modalCategory"}>
      <div
        className={active ? "modalCategory_content active" : "modalCategory_content"}>
        <button className="modal_close1" onClick={() => setActive(false)}>
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="modal_close_x1"
               viewBox="0 0 16 16">
            <path
