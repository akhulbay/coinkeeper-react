...
export function getUserInfo(userInfo) {
  return userInfo
    ? {
        accounts: [
          createAccount("Cash", 0, storage.iconsForAccount[2]),
          createAccount("Kaspi", 0, storage.iconsForAccount[1]),
        ],
        incomes: [
          createIncome("IITU", 100000, storage.iconsForIncome[1]),
          createIncome("Salary", 150000, storage.iconsForIncome[4]),
        ],
...
export function createIncome(title: string, plannedIncome: number, icon) {
  return {
    id: uuid(),
    title: title,
...
export function createAccount(title, initialBalance, icon) {
  return {
    id: uuid(),
    title: title,
...
export function getAccountCurrentBalance(
  ...
  if (incomeTransactionList != [] && outcomeTransactionList != []) {
    return Math.round(incomeTransactionAmount - outcomeTransactionAmount);
  } else {
    return Math.round(0);
  }
}
...
export function getTransactionType(source, destination) {
  if (isIncomeSource(source) && isAccount(destination))
    return TransactionType.Income;
  if (isAccount(source) && isAccount(destination))
    return TransactionType.Transfer;
  if (isAccount(source) && isExpenseType(destination))
